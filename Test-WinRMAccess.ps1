...
function Main($ComputerName, $ShowListener, $ShowHTTPSCertificate, $ShowAllCertificates, $Summary, $TestPSSessionSSL) {
	...
	$WinRMStateInfo = "" | Select-Object FQDN, RealName, IPAddress, HTTP, HTTPS, Certificate, CorrectCert, ThumbPrint, CandidateTP, Connection, MachineType, SSLPSSession, GPODefined, Compatibility
	...
	$CIMName = (Get-CimInstance -ClassName Win32_ComputerSystem -CimSession $CimSession).Name
	$CIMDomain = (Get-CimInstance -ClassName Win32_ComputerSystem -CimSession $CimSession).Domain
...
main -ComputerName $ComputerName -ShowListener $ShowListener -ShowHTTPSCertificate $ShowHTTPSCertificate -ShowAllCertificates $ShowAllCertificates -Summary $Summary -TestPSSessionSSL $TestPSSessionSSL
